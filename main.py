...
def extract_close_series(raw_data, ticker):
    if len(raw_data.columns.names) > 1:
...
prev = close_df.ffill().iloc[-2] if len(close_df) > 1 else latest
daily_change = ((latest / prev) - 1) * 100
period_return = ((close_df.ffill().iloc[-1] / close_df.ffill().iloc[0]) - 1) * 100

st.subheader("시장 요약")

metric_cols = st.columns(min(4, len(close_df.columns)))
for i, col_name in enumerate(close_df.columns[:4]):
...
with tab3:
    ...
    if original_ticker:
        hist_5d, fast_info = load_fast_info(original_ticker)

        c1, c2, c3, c4 = st.columns(4)
        last_price = hist_5d["Close"].iloc[-1] if not hist_5d.empty else None
        open_price = hist_5d["Open"].iloc[-1] if not hist_5d.empty else None
        high_price = hist_5d["High"].iloc[-1] if not hist_5d.empty else None
        low_price = hist_5d["Low"].iloc[-1] if not hist_5d.empty else None
        ...
        detail_df = close_df[[selected_detail]].dropna().copy()
        detail_df["20D_MA"] = detail_df[selected_detail].rolling(20).mean()
        detail_df["60D_MA"] = detail_df[selected_detail].rolling(60).mean()

        fig_detail = go.Figure()
        fig_detail.add_trace(
            go.Scatter(
                x=detail_df.index,
                y=detail_df[selected_detail],
                mode="lines",
                name="Close",
                line=dict(width=2),
            )
